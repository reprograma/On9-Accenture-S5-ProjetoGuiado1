$(document).ready(function () {
  ...
  function filter() {
    ...
    const filteredList = filterPokemon(name, type, pokemonList);
    load(filteredList);
  ...
  function load(pokedex) {
    const view = pokedex.map((p) => createCard(p)).join("");
    $(".pokedex").html(
      view + '<div class="warning hidden">Nenhum pokemon foi encontrado.</div>'
    );
  ...
  function createCard(pokemon) {
    ...
    return `<div class="pokemon" data-name="${
      pokemon.name
    ...
    <div class="btn-toolbar float-right mt-2" role="toolbar" aria-label="Toolbar with button groups">
      <div class="btn-group btn-group-sm mr-2" role="group" aria-label="First group">
        <button type="button" class="btn btn-link btn-edit" data-id="${
          pokemon.id
        }">
        ...
        <button type="button" class="btn btn-link btn-trash" data-id="${
          pokemon.id
        }">
  ...
  function loadTypes(types) {
    ...
    types.map((t) =>
      $(".ckb-types").append(
        `<div class="form-check form-check-inline"><input class="form-check-input type" type="checkbox"" value="${t}"><label class="form-check-label">${t}</label></div>`
      )
    );
  }
